...
async def check_auth_employer_token(token: Annotated[str, Depends(token_header)])\
    -> GetEmployerByTokenResult:
    return await get_employer_by_token(token)

async def check_auth_worker_token(token: Annotated[str, Depends(token_header)])\
